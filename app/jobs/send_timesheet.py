...
def get_todays_tasks():
    ...
    try:
        ...
        logger.error(f"Error: Worksheet '{WORKSHEET_TITLE}' not found in spreadsheet '{SPREADSHEET_NAME}'.")
        return [], None
...
async def main():
    ...
    if is_holiday:
        logger.info(f"Today is {holiday_name}. Skipping Timesheet sending.")
        return

    tasks, today_str = get_todays_tasks()
    
    if tasks:
        logger.info(f"Found {len(tasks)} tasks for today ({today_str}). Sending to Telegram...")
        await send_telegram_messages(tasks)
    else:
        logger.info(f"No tasks found for today ({today_str}). Skipping Telegram.")
